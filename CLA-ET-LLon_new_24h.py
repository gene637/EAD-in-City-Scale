...
def run(save_folder):
    ...
    total_electricity_other = np.zeros(48)
    total_second_other = np.zeros(48)
    total_meter_other = np.zeros(48)
    total_electricity_caev = np.zeros(48)
    total_second_caev = np.zeros(48)
    total_meter_caev = np.zeros(48)

    while traci.simulation.getMinExpectedNumber() > 0:
        traci.simulationStep()
        time = traci.simulation.getTime()

        halfhour = int(step//1800)
        # 7 a.m. scenario
            ...
            if EV == 'on':
                if not np.isnan(traci.vehicle.getElectricityConsumption(veh_id)):
                    if traci.vehicle.getTypeID(veh_id) == "EP":
                        ...
                        total_electricity_caev[halfhour] += electricity
                        total_second_caev[halfhour] += second
                        total_meter_caev[halfhour] += meter
                        if EAD == 'on':
                        ...
                        second = 1
                        meter = traci.vehicle.getSpeed(veh_id)
                        total_electricity_other[halfhour] += electricity
                        total_second_other[halfhour] += second
                        total_meter_other[halfhour] += meter
    ...
    for i in range(48):
        hours = int(i//2)
        minutes = (i-hours*2)*30
        filename = save_folder+str(hours)+'_'+str(minutes)+'_Electricity.txt'
        with open(filename, 'w') as file:
            # Write directly to the file
            file.write(f"Total elecctricity consumption: {total_electricity_other[i] + total_electricity_caev[i]} Wh\n")
            file.write(f"caepv elecctricity consumption: {total_electricity_caev[i]} Wh\n")
            file.write(f"other ev elecctricity consumption: {total_electricity_other[i]} Wh\n")
            file.write(f"Total travel time in hour (VHT): {(total_second_other[i] + total_second_caev[i])/3600} h\n")
            file.write(f"cav travel time in hour (VHT): {total_second_caev[i]/3600} h\n")
            file.write(f"other ev travel time in hour (VHT): {total_second_other[i]/3600} h\n")
            file.write(f"Total travel miles: {(total_meter_other[i] + total_meter_caev[i])/1609.34} miles\n")
            file.write(f"cav travel miles: {total_meter_caev[i]/1609.34} miles\n")
            file.write(f"other travel miles: {total_meter_other[i]/1609.34} miles\n")
...
if __name__ == "__main__":
    ...
    CAEPVPr = 15/100
    save_folder = '/home/gene/Downloads/EAD code test/24_hour_toyota_'+str(CAEPVPr)+'caep_static/'

    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    path = '/home/gene/numo/routes/'
    new_path = '/home/gene/numo/new_routes_0.1caep/'
    generate_routefile_cav(path, new_path)

    traci.start([sumoBinary, "-c", "/home/gene/numo/nagoya_0.1caep_static_tls.sumocfg", "--step-length", "1", "--device.rerouting.threads",
