...
EAD_filename = 'EAD_' + str(datetime.datetime.now().date()) + '_' + str(datetime.datetime.now().time()).replace(':', '_')
f2 = open(EAD_filename + ".txt", "w")
f2.close()


#  09/29/2021 David Oswald: changed function to have angle and new phase times (nPT).
...
def EAD_acceleration(veh_position, state, next_switch, simulation_time, speed, ID, last_speed, angle, nPT, vehType, vehQs, prl):
            ...
            if 20 <= dist <= 750:
                ...
                if vehType == "typeCAV":
                    if 50 < dist < 200:
                        if lT > 1:
                            if 0 < lK < (lT - 1):
                                if vehQs[pK][rK][lK] <= vehQs[pK][rK][lK + 1] and vehQs[pK][rK][lK] <= vehQs[pK][rK][lK - 1]:
                                    traci.vehicle.changeLane(ID, lK, 50)
                                else:
                                    if vehQs[pK][rK][lK + 1] <= vehQs[pK][rK][lK - 1]:
                        ...
                        traci.vehicle.changeLane(ID, lK, 50)
                # if vehType == "typeCAV_left" or vehType == "typeCAV_R":
